    ...
    <div className="container justify-content-between">
      ...
      <div className="collapse navbar-collapse flex-grow-0" id="navbar">
        <ul className="navbar-nav mb-2 mb-lg-0">
          {appContext.token && <li className="nav-item">
            <Link to='/' className={"nav-link " + (location.pathname === '/' && 'active')}>Home</Link>
          </li>}
          {appContext.token && <li className="nav-item" >
            <Link to='/files' className={"nav-link " + (location.pathname === '/files' && 'active')}>Files</Link>
          </li>}
          {appContext.token && <li className="nav-item">
...
export function PageTitle({ title }: { title: string }) {
  return <>
    <h1 className='mt-4 mb-5 display-7 fs-1 text-center text-md-start fw-bold'> {title} </h1>
  </>
}
